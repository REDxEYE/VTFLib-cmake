...
#include "../include/VTFFile.h"
#include "../include/VTFFormat.h"
#include "../include/VTFDXTn.h"
#include "../include/VTFMathlib.h"
                    ...
                    for (vlUInt k = 0; k < uiSlices; k++) {
                        for (vlUInt l = 0; l < uiMipmaps; l++) {
                            ...
                            this->Convert(VTFFile.GetData(i, j, k, l), this->GetData(i, j, k, l), uiMipmapWidth,
                                          uiMipmapHeight, VTFFile.GetFormat(), this->GetFormat());
                        }
                    }
                }
            }
...
// generated.
//
vlBool CVTFFile::Create(vlUInt uiWidth, vlUInt uiHeight, vlUInt uiFrames, vlUInt uiFaces, vlUInt uiSlices,
                        VTFImageFormat ImageFormat, vlBool bThumbnail, vlBool bMipmaps, vlBool bNullImageData) {
    this->Destroy();
    ...
    if (!this->IsPowerOfTwo(uiWidth) || uiWidth > 0xffff) {
        if (uiWidth == 0) {
            ...
            LastError.SetFormatted(
                    "Invalid image width %u.  Width must be a power of two (nearest powers are %u and %u).", uiWidth,
                    uiNextPowerOfTwo >> 1, uiNextPowerOfTwo);
        }
        return vlFalse;
    }
    ...
    if (!this->IsPowerOfTwo(uiHeight) || uiHeight > 0xffff) {
        if (uiHeight == 0) {
            ...
            LastError.SetFormatted(
                    "Invalid image height %u.  Height must be a power of two (nearest powers are %u and %u).", uiHeight,
                    uiNextPowerOfTwo >> 1, uiNextPowerOfTwo);
        }
        return vlFalse;
    }
    ...
    if (!this->IsPowerOfTwo(uiSlices) || uiSlices > 0xffff) {
        if (uiHeight == 0) {
            ...
            LastError.SetFormatted(
                    "Invalid image depth %u.  Depth must be a power of two (nearest powers are %u and %u).", uiSlices,
                    uiNextPowerOfTwo >> 1, uiNextPowerOfTwo);
        }
        return vlFalse;
    }
    ...
    if (uiFaces != 1 && uiFaces != 6 && VTF_MINOR_VERSION_DEFAULT >= VTF_MINOR_VERSION_MIN_NO_SPHERE_MAP) {
        LastError.SetFormatted("Invalid image face count %u for version %d.%d.", uiFaces, VTF_MAJOR_VERSION,
    ...
    this->Header->StartFrame =
            uiFaces != 6 || VTF_MINOR_VERSION_DEFAULT >= VTF_MINOR_VERSION_MIN_NO_SPHERE_MAP ? 0 : 0xffff;
    ...
    this->uiImageBufferSize = this->ComputeImageSize(this->Header->Width, this->Header->Height, this->Header->Depth,
                                                     this->Header->MipCount, this->Header->ImageFormat) * uiFrames *
                              uiFaces;
    this->lpImageData = new vlByte[this->uiImageBufferSize];
...
vlBool CVTFFile::Create(vlUInt uiWidth, vlUInt uiHeight, vlUInt uiFrames, vlUInt uiFaces, vlUInt uiSlices,
    ...
    if ((uiFrames == 1 && uiFaces > 1 && uiSlices > 1) || (uiFrames > 1 && uiFaces == 1 && uiSlices > 1) ||
        (uiFrames > 1 && uiFaces > 1 && uiSlices == 1)) {
        LastError.Set(
    ...
    if (VTFCreateOptions.uiVersion[0] != VTF_MAJOR_VERSION ||
        ...
        LastError.SetFormatted("File version %u.%u does not match %d.%d to %d.%d.", VTFCreateOptions.uiVersion[0],
    ...
    if (VTFCreateOptions.uiVersion[0] == VTF_MAJOR_VERSION &&
        VTFCreateOptions.uiVersion[1] < VTF_MINOR_VERSION_MIN_VOLUME && uiSlices > 1) {
        LastError.SetFormatted("Volume textures are only supported in version %d.%d and up.", VTF_MAJOR_VERSION,
                               VTF_MINOR_VERSION_MIN_VOLUME);
        return vlFalse;
    }

    if (VTFCreateOptions.uiVersion[0] == VTF_MAJOR_VERSION &&
        VTFCreateOptions.uiVersion[1] < VTF_MINOR_VERSION_MIN_SPHERE_MAP && uiFaces == 7) {
        LastError.SetFormatted("Sphere maps are only supported in version %d.%d and up.", VTF_MAJOR_VERSION,
    ...
    try {
        if (VTFCreateOptions.bResize) {
            ...
            if (uiWidth != uiNewWidth || uiHeight != uiNewHeight) {
                ...
                for (vlUInt i = 0; i < uiCount; i++) {
                    lpNewImageDataRGBA8888[i] = new vlByte[this->ComputeImageSize(uiNewWidth, uiNewHeight, 1,
                                                                                  IMAGE_FORMAT_RGBA8888)];

                    if (!this->Resize(lpImageDataRGBA8888[i], lpNewImageDataRGBA8888[i], uiWidth, uiHeight, uiNewWidth,
                                      uiNewHeight, VTFCreateOptions.ResizeFilter,
                                      VTFCreateOptions.ResizeSharpenFilter)) {
                        throw 0;
                    }
                }
        ...
        if (!this->Create(uiWidth, uiHeight, uiFrames, uiFaces + (VTFCreateOptions.bSphereMap && uiFaces == 6 ? 1 : 0),
                          uiSlices, VTFCreateOptions.ImageFormat, VTFCreateOptions.bThumbnail,
                          VTFCreateOptions.bMipmaps, vlFalse)) {
            throw 0;
        }
        ...
        if (VTFCreateOptions.bNormalMap && uiFaces == 1) {
            ...
            for (vlUInt i = 0; i < uiFrames; i++) {
                for (vlUInt j = 0; j < uiFaces; j++) {
                    for (vlUInt k = 0; k < uiSlices; k++) {
                        if (!this->ConvertToNormalMap(lpImageDataRGBA8888[i + j + k], 0/*lpImageDataNormalMap*/,
                                                      this->Header->Width, this->Header->Height,
                                                      VTFCreateOptions.KernelFilter,
                                                      VTFCreateOptions.HeightConversionMethod,
                                                      VTFCreateOptions.NormalAlphaResult,
                                                      VTFCreateOptions.bNormalMinimumZ, VTFCreateOptions.sNormalScale,
                                                      VTFCreateOptions.bNormalWrap, VTFCreateOptions.bNormalInvertX,
                                                      VTFCreateOptions.bNormalInvertY,
                                                      VTFCreateOptions.bNormalInvertZ)) {
...
vlBool CVTFFile::Save(vlVoid* pUserData) const {
    auto writer = IO::Writers::CProcWriter(pUserData);
...
vlBool CVTFFile::Load(IO::Readers::IReader* Reader, vlBool bHeaderOnly) {
    ...
    try {
        ...
        if (FileHeader.Version[0] != VTF_MAJOR_VERSION ||
            ...
            LastError.SetFormatted("File version %u.%u does not match %d.%d to %d.%d.", FileHeader.Version[0],
                                   FileHeader.Version[1], VTF_MAJOR_VERSION, 0, VTF_MAJOR_VERSION, VTF_MINOR_VERSION);
            throw 0;
        }

        if (FileHeader.HeaderSize > sizeof(SVTFHeader)) {
            LastError.SetFormatted("File header size %d B is larger than the %d B maximum expected.",
        ...
        if (this->Header->Version[0] < VTF_MAJOR_VERSION || (this->Header->Version[0] == VTF_MAJOR_VERSION &&
                                                             this->Header->Version[1] < VTF_MINOR_VERSION_MIN_VOLUME)) {
        ...
        this->uiImageBufferSize = this->ComputeImageSize(this->Header->Width, this->Header->Height, this->Header->Depth,
                                                         this->Header->MipCount, this->Header->ImageFormat) *
        ...
        if (this->Header->HeaderSize > uiFileSize ||
            uiThumbnailBufferOffset + this->uiThumbnailBufferSize > uiFileSize ||
            uiImageDataOffset + this->uiImageBufferSize > uiFileSize) {
    ...
    try {
        ...
        if (this->GetSupportsResources()) {
            for (vlUInt i = 0; i < this->Header->ResourceCount; i++) {
                switch (this->Header->Resources[i].Type) {
                    case VTF_LEGACY_RSRC_LOW_RES_IMAGE:
                        if (Writer->Write(this->lpThumbnailImageData, this->uiThumbnailBufferSize) !=
                    ...
                    default:
                        if ((this->Header->Resources[i].Flags & RSRCF_HAS_NO_DATA_CHUNK) == 0) {
                            ...
                            if (Writer->Write(this->Header->Data[i].Data, this->Header->Data[i].Size) !=
                                this->Header->Data[i].Size) {
                                throw 0;
                            }
                        }
                }
            ...
            switch (this->Header->Version[1]) {
                ...
                case 3:
                    this->Header->HeaderSize =
                            sizeof(SVTFHeader_73_A) + this->Header->ResourceCount * sizeof(SVTFResource);
                    break;
                case 4:
                    this->Header->HeaderSize =
                            sizeof(SVTFHeader_74_A) + this->Header->ResourceCount * sizeof(SVTFResource);
                    break;
                case 5:
                    this->Header->HeaderSize =
                            sizeof(SVTFHeader_75_A) + this->Header->ResourceCount * sizeof(SVTFResource);
                    break;
            }
            break;
    }
...
vlUInt CVTFFile::GetFaceCount() const {
    ...
    return this->Header->Flags & TEXTUREFLAGS_ENVMAP ? (this->Header->StartFrame != 0xffff &&
                                                        this->Header->Version[1] < VTF_MINOR_VERSION_MIN_NO_SPHERE_MAP
                                                        ? CUBEMAP_FACE_COUNT : CUBEMAP_FACE_COUNT - 1) : 1;
}
...
vlVoid CVTFFile::SetFlag(VTFImageFlag ImageFlag, vlBool bState) {
    ...
    // Don't let the user set flags critical to the image's format.
    if (ImageFlag == TEXTUREFLAGS_ONEBITALPHA || ImageFlag == TEXTUREFLAGS_EIGHTBITALPHA ||
        ImageFlag == TEXTUREFLAGS_ENVMAP) {
        return;
    }
...
vlByte* CVTFFile::GetData(vlUInt uiFrame, vlUInt uiFace, vlUInt uiSlice, vlUInt uiMipmapLevel) const {
    ...
    return this->lpImageData +
           this->ComputeDataOffset(uiFrame, uiFace, uiSlice, uiMipmapLevel, this->Header->ImageFormat);
}
...
vlVoid CVTFFile::SetData(vlUInt uiFrame, vlUInt uiFace, vlUInt uiSlice, vlUInt uiMipmapLevel, vlByte* lpData) {
    ...
    memcpy(this->lpImageData +
           this->ComputeDataOffset(uiFrame, uiFace, uiSlice, uiMipmapLevel, this->Header->ImageFormat), lpData,
           CVTFFile::ComputeMipmapSize(this->Header->Width, this->Header->Height, 1, uiMipmapLevel,
                                       this->Header->ImageFormat));
}
...
vlVoid CVTFFile::SetThumbnailData(vlByte* lpData) {
    ...
    memcpy(this->lpThumbnailImageData, lpData,
           this->uiThumbnailBufferSize/*CVTFFile::ComputeImageSize(this->Header->LowResImageWidth, this->Header->LowResImageHeight, this->Header->LowResImageFormat)*/);
}
...
vlBool CVTFFile::GetSupportsResources() const {
    ...
    return this->Header->Version[0] > VTF_MAJOR_VERSION || (this->Header->Version[0] == VTF_MAJOR_VERSION &&
        ...
        if (this->GetSupportsResources()) {
            switch (uiType) {
                ...
                default:
                    ...
                    if (uiSize != 0) {
                        if (this->Header->ResourceCount == VTF_RSRC_MAX_DICTIONARY_ENTRIES) {
                            LastError.SetFormatted("Maximum directory entry count %u reached.",
...
//
vlBool
CVTFFile::GenerateMipmaps(vlUInt uiFace, vlUInt uiFrame, VTFMipmapFilter MipmapFilter, VTFSharpenFilter SharpenFilter) {
    if (!this->IsLoaded())
        return vlFalse;
...
vlBool CVTFFile::GenerateThumbnail() {
    ...
    for (vlUInt i = 0; i < this->Header->MipCount; i++) {
        vlUInt uiMipmapWidth, uiMipmapHeight, uiMipmapDepth;
        CVTFFile::ComputeMipmapDimensions(this->Header->Width, this->Header->Height, 1, i, uiMipmapWidth,
                                          uiMipmapHeight, uiMipmapDepth);

        if (uiMipmapWidth == (vlUInt) this->Header->LowResImageWidth &&
            uiMipmapHeight == (vlUInt) this->Header->LowResImageHeight) {
            ...
            if (this->Header->ImageFormat == this->Header->LowResImageFormat) {
                ...
                if (!CVTFFile::Convert(this->GetData(0, 0, 0, i), this->GetThumbnailData(), uiMipmapWidth,
                                       uiMipmapHeight, this->Header->ImageFormat, this->Header->LowResImageFormat)) {
                    return vlFalse;
                }
            }
            return vlTrue;
    ...
    vlByte* lpImageData = new vlByte[CVTFFile::ComputeImageSize(this->Header->Width, this->Header->Height, 1,
                                                                IMAGE_FORMAT_RGBA8888)];
    vlByte* lpThumbnailImageData = new vlByte[CVTFFile::ComputeImageSize(this->Header->LowResImageWidth,
                                                                         this->Header->LowResImageHeight, 1,
                                                                         IMAGE_FORMAT_RGBA8888)];

    if (!CVTFFile::ConvertToRGBA8888(this->GetData(0, 0, 0, 0), lpImageData, this->Header->Width, this->Header->Height,
                                     this->Header->ImageFormat)) {
        delete[]lpImageData;
    ...
    if (!CVTFFile::Resize(lpImageData, lpThumbnailImageData, this->Header->Width, this->Header->Height,
                          this->Header->LowResImageWidth, this->Header->LowResImageHeight)) {
        delete[]lpImageData;
        delete[]lpThumbnailImageData;
    ...
    if (!CVTFFile::ConvertFromRGBA8888(lpThumbnailImageData, this->GetThumbnailData(), this->Header->LowResImageWidth,
                                       this->Header->LowResImageHeight, this->Header->LowResImageFormat)) {
        delete[]lpImageData;
        delete[]lpThumbnailImageData;
...
//
vlBool CVTFFile::GenerateNormalMap(VTFKernelFilter KernelFilter, VTFHeightConversionMethod HeightConversionMethod,
...
//
vlBool CVTFFile::GenerateNormalMap(vlUInt uiFrame, VTFKernelFilter KernelFilter,
    ...
    if (!this->ConvertToNormalMap(lpSource, 0/*lpDest*/, this->Header->Width, this->Header->Height, KernelFilter,
        ...
        for (vlUInt uiFace = 0; uiFace < uiFaceCount; uiFace++) {
            for (vlUInt uiSlice = 0; uiSlice < uiSliceCount; uiSlice++) {
                if (!this->ConvertToRGBA8888(this->GetData(uiFrame, uiFace, uiSlice, 0), lpImageData,
                                             this->Header->Width, this->Header->Height, this->Header->ImageFormat)) {
                    delete[]lpImageData;
...
vlUInt CVTFFile::ComputeMipmapSize(vlUInt uiWidth, vlUInt uiHeight, vlUInt uiDepth, vlUInt uiMipmapLevel,
    ...
    vlUInt uiMipmapWidth, uiMipmapHeight, uiMipmapDepth;
    CVTFFile::ComputeMipmapDimensions(uiWidth, uiHeight, uiDepth, uiMipmapLevel, uiMipmapWidth, uiMipmapHeight,
                                      uiMipmapDepth);
...
//
vlBool
CVTFFile::CompressDXTn(vlByte* lpSource, vlByte* lpDest, vlUInt uiWidth, vlUInt uiHeight, VTFImageFormat DestFormat) {
#ifdef USE_NVDXT
                                                                                                                            nvCompressionOptions Options = nvCompressionOptions();
...
vlVoid ToLuminance(vlUInt16& R, vlUInt16& G, vlUInt16& B, vlUInt16& A) {
    R = G = B = (vlUInt16) (sLuminanceWeightR * (vlSingle) R + sLuminanceWeightG * (vlSingle) G +
                            sLuminanceWeightB * (vlSingle) B);
}
...
vlVoid
Transform(TransformProc pTransform1, TransformProc pTransform2, T SR, T SG, T SB, T SA, T SRBits, T SGBits, T SBBits,
          T SABits, U& DR, U& DG, U& DB, U& DA, U DRBits, U DGBits, U DBBits, U DABits) {
    ...
    if (SourceInfo.bIsCompressed || DestInfo.bIsCompressed) {
        ...
        switch (SourceFormat) {
            ...
            case IMAGE_FORMAT_DXT5:
                ...
                break;
            default:
                bResult = CVTFFile::Convert(lpSource, lpSourceRGBA, uiWidth, uiHeight, SourceFormat,
                                            IMAGE_FORMAT_RGBA8888);
                break;
        }
        ...
        if (bResult) {
            ...
            switch (DestFormat) {
                ...
                case IMAGE_FORMAT_DXT5:
                    ...
                    break;
                default:
                    bResult = CVTFFile::Convert(lpSourceRGBA, lpDest, uiWidth, uiHeight, IMAGE_FORMAT_RGBA8888,
                                                DestFormat);
                    break;
            }
        }
...
//
vlBool CVTFFile::ConvertToNormalMap(vlByte* lpSourceRGBA8888, vlByte* lpDestRGBA8888, vlUInt uiWidth, vlUInt uiHeight,
                                    VTFKernelFilter KernelFilter, VTFHeightConversionMethod HeightConversionMethod,
                                    VTFNormalAlphaResult NormalAlphaResult, vlByte bMinimumZ, vlSingle sScale,
                                    vlBool bWrap, vlBool bInvertX, vlBool bInvertY, vlBool bInvertZ) {
...
vlBool CVTFFile::Resize(vlByte* lpSourceRGBA8888, vlByte* lpDestRGBA8888, vlUInt uiSourceWidth, vlUInt uiSourceHeight,
                        vlUInt uiDestWidth, vlUInt uiDestHeight, VTFMipmapFilter ResizeFilter,
...
//
vlVoid
CVTFFile::CorrectImageGamma(vlByte* lpImageDataRGBA8888, vlUInt uiWidth, vlUInt uiHeight, vlSingle sGammaCorrection) {
...
//
vlVoid CVTFFile::ComputeImageReflectivity(vlByte* lpImageDataRGBA8888, vlUInt uiWidth, vlUInt uiHeight, vlSingle& sX,
