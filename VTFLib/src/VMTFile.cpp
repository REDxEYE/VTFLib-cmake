...
public:
    ...
    CToken(vlChar* lpString, vlBool bQuoted) : eToken(bQuoted ? TOKEN_QUOTED_STRING : TOKEN_STRING), cChar('\0'),
                                               lpString(0) {
    ...
    vlVoid Parse(CVMTGroupNode* Group) {
        ...
        while (true) {
            ...
            if (Token->GetToken() == TOKEN_CLOSE_BRACE ||
                (uiVMTParseMode == PARSE_MODE_LOOSE && Token->GetToken() == TOKEN_EOF)) {
                return;
            }
